...
/**
 * GroupSessionBuilder is responsible for setting up group SenderKey encrypted
 * sessions.
 *
 * Once a session has been established,
 * {@link org.whispersystems.libsignal.groups.GroupCipher} can be used to
 * encrypt/decrypt messages in that session.
 * <p>
 * The built sessions are unidirectional: they can be used either for sending or
 * for receiving, but not both.
 *
 * Sessions are constructed per (groupId + senderId + deviceId) tuple. Remote
 * logical users are identified by their senderId, and each logical recipientId
 * can have multiple physical devices.
 *
...
public class GroupSessionBuilder {
     ...
     *
     * @param sender The address associated with the
     * SenderKeyDistributionMessage.
     * @param senderKeyDistributionMessage A received
     * SenderKeyDistributionMessage.
     */
    public void process(SignalProtocolAddress sender, SenderKeyDistributionMessage senderKeyDistributionMessage) {
        synchronized (GroupCipher.LOCK) {
            ...
            logger.fine("Process GroupSessionBuilder for UUID = " + uuid);
     ...
     *
     * @param senderKeyName The (groupId, senderId, deviceId) tuple. In this
     * case, 'senderId' should be the caller.
     * @return A SenderKeyDistributionMessage that is individually distributed
